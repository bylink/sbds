<template>
  <div class="el-grid-col" :class="classes" :style="styles"><slot></slot></div>
</template>
...
export default {
  ...
  computed: {
    classes() {
      const list = {}
      values.forEach(value => {
        list[`el-grid-col--${value}-${this[value]}`] = this[value]
        sizes.forEach(size => {
          const sizeValue = this[`${value}${_upperFirst(size)}`]
          list[`el-grid-col--${value}-${size}-${sizeValue}`] = sizeValue
        })
      })
      return list
    },
